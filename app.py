...
if "FLUSH_INTERVAL" in os.environ:
    # check if numeric and if it's under 900 seconds we will use it. If > 900 seconds we will use 900 seconds and log a warning of config out of range
    if os.environ["FLUSH_INTERVAL"].isdigit():
        if int(os.environ["FLUSH_INTERVAL"]) < 900:
            flush_interval = int(os.environ["FLUSH_INTERVAL"])
        else:
            flush_interval = 900
            logging.warning("FLUSH_INTERVAL env variable is set to %s, which is out of range, using 900 seconds instead", os.environ["FLUSH_INTERVAL"])
